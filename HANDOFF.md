...
16. **INTERMEDIAÇÃO — Fase 1 (2026-09-11, PRD v4 do Marco):** migration `20260911200000_intermediacao_nucleo.sql` APLICADA (em 2 partes via MCP). Entidade-mãe `intermediations` (INT-xxxxx; termos comerciais, contrato, prazo, estados alternativos, comissão) + `intermediation_events` + `legal_entities` (seed Totex Digital Mídia). Funil renomeado **"Intermediação Tamboré"** com 8 macroetapas + Encerrada; gate `trg_intermediation_deal_gate` (sem contrato não passa de Contratar; Concluída só automática). **R$ 25 mudou de gatilho:** agora é `intermediation_formalized` (contrato assinado — na fase 1, importado pelo admin com PDF+SHA-256+motivo); R$ 50 = `intermediation_completed`. Cron `intermediation-deadlines` (09:15 BRT) avisa prazo. UI: `IntermediationCard` no lead, aba Funil em Prêmios, promotora vê "Intermediações". Decisões: Clicksign na fase 3, funil de 8. Plano das fases 2–6 em `docs/INTERMEDIACAO.md`.

17. **INTERMEDIAÇÃO — Fase 2 Documentos (2026-09-12):** migration `20260912100000_intermediacao_contratos.sql` APLICADA. `contract_templates` (versionado, imutável quando publicado; v1 global = texto do PRD), `contract_documents` + `contract_signers` (cada geração = versão; regenerar cancela a anterior), dados do contrato (`intermediations.owner_data`, `seller_vehicles.plate/renavam/chassis`, `legal_entities.signer_*`), RPCs `intermediation_contract_snapshot` (variáveis + faltantes) / `intermediation_set_contract_data` / `contract_document_register` (service_role). Edge fn **`contract-render`** (JWT; pdf-lib; `{preview:true}` não grava). Chaves `CLICKSIGN_API_KEY`/`CLICKSIGN_ENV` em API Keys. Pendência: CPF de quem assina pela Totex (Configurações › Comercial › Intermediação). Fase 3 = adapter + Clicksign + webhook.

18. **INTERMEDIAÇÃO — Fase 3 Assinatura eletrônica (2026-09-13):** migration `20260913100000_intermediacao_assinatura.sql` APLICADA (2 partes). `contract_events` (trilha bruta + idempotência), campos de provedor em `contract_documents`/`contract_signers`, `legal_entities.signer_email/phone`. **`intermediation_formalize()`** é a rotina única (import manual e assinatura): `active`, R$ 25 idempotente, deal → Preparação. RPCs service_role: `contract_document_mark_sent`, `contract_apply_provider_event` (fora de ordem, duplicado, terminal não regride), `contract_document_finalize` (só depois do PDF assinado baixado + SHA-256), `contract_document_set_status`, `contract_documents_to_reconcile`, `contract_document_by_provider_ref`. Adapter `_shared/signature/*` (Clicksign API v3). Edge fns **`contract-send`** (JWT; send/resend/cancel/status/register_webhook/test_connection), **`clicksign-webhook`** (sem JWT; HMAC `Content-Hmac` com `CLICKSIGN_WEBHOOK_SECRET` do tenant), **`contract-reconcile`** (cron `contract-reconcile` */30). UI: dialog "Enviar para assinatura" no `IntermediationCard` (canal/autenticação por signatário, prazo), chips de status, reenviar/verificar/cancelar, eventos; Configurações › Integrações: `CLICKSIGN_ENV` select + "Testar conexão" + "Registrar webhook". **Para ativar:** (1) confirmar se o token do Marco é sandbox ou produção e ajustar `CLICKSIGN_ENV`; (2) clicar "Registrar webhook" (grava o secret); (3) preencher e-mail/telefone de quem assina pela Totex; (4) teste ponta a ponta em sandbox. Detalhes em `docs/INTERMEDIACAO.md`.

19. **Consulta por placa (PuxaPlaca) (2026-09-12):** migration `20260912120000_consulta_placa.sql` APLICADA. Edge fn **`vehicle-lookup`** (JWT; portada de `totexcar-copilot/vehicle-lookup`): provedor PuxaPlaca (`GET /v2/consulta/{placa}`, header `token`; legado API Brasil se `PUXAPLACA_URL` apontar pra lá), chave por tenant `PUXAPLACA_TOKEN`, cache por (tenant, placa) 30 dias em `vehicle_plate_lookups` (cada consulta é cobrada), limite diário por membro (promotora 20 / demais 100), promotora recebe só marca/modelo/ano/cor/combustível (chassi/Renavam só admin/comercial/closer), avisa se a placa já foi captada (`seller_vehicle_by_plate`). UI: campo Placa + "Buscar" em `CaptureNewLead` (opcional; autopreenche), botão "Consultar placa" nos Dados do contrato do `IntermediationCard` (preenche chassi/Renavam também). `create_capture_lead` grava `plate/plate_last4/color/fuel`. Token do tenant Totex já gravado. Hook `useVehicleLookup`. Placa é opcional (confirmado). O Marco subiu o limite de Edge Functions no Supabase (2026-09-12) e a `vehicle-lookup` foi restaurada na esteira e deployada. Recurso completo em produção.

20. **INTERMEDIAÇÃO — Fase 4 Comprador e Pagamento (2026-09-14):** migration `20260914100000_intermediacao_fechamento.sql` APLICADA (2 partes via MCP). `intermediations` ganhou comprador/venda/pagamento (buyer_data, payment_method, down_payment, financed_amount, installments, paid_amount, payment_confirmed_*, sale_document_id, sale_contract_status, sale_signed_at). Tabela `intermediation_proposals` (histórico + aceite). Termo de Compra e Venda = `document_type='SALE_CONTRACT'` reusando `contract-render`/`contract-send`/webhook/`finalize` (agora conscientes do document_type; SALE_CONTRACT tem 3º signatário `buyer` e NÃO paga R$25). RPCs `intermediation_set_buyer`/`add_proposal`/`decide_proposal`/`confirm_payment`/`conclude_sale`/`import_sale_contract`. **`conclude_sale` trava:** exige termo assinado + pagamento confirmado → carro vendido → R$50 (triggers Fase 1). Template `SALE_CONTRACT` v1 global publicado (**rascunho, revisar no jurídico**). Credere = fase 4.1. UI da Fase 4 (bloco Fechamento no IntermediationCard) sendo construída por agente. Smoke: `scratchpad/smoke_test11.sql`. Detalhes em `docs/INTERMEDIACAO.md`.

21. **INTERMEDIAÇÃO — Painel de gestão (2026-09-14):** RPC `intermediation_dashboard(period)` (migration `20260914160000_intermediacao_painel.sql`, aplicada) agrega KPIs, filas "precisa de atenção" (prazos, contratos/termos pendentes, pagamentos, prontas p/ concluir), ranking de promotoras e prêmios por status — 1 chamada, admin/comercial (promotora barrada). UI: página `src/pages/IntermediationDashboard.tsx` na rota `/comercial/intermediacao` + item "Intermediação" na sidebar comercial; listas clicáveis levam ao lead. Hook `useIntermediationDashboard`.
