...
19. **Consulta por placa (PuxaPlaca) (2026-09-12):** migration `20260912120000_consulta_placa.sql` APLICADA. Edge fn **`vehicle-lookup`** (JWT; portada de `totexcar-copilot/vehicle-lookup`): provedor PuxaPlaca (`GET /v2/consulta/{placa}`, header `token`; legado API Brasil se `PUXAPLACA_URL` apontar pra lá), chave por tenant `PUXAPLACA_TOKEN`, cache por (tenant, placa) 30 dias em `vehicle_plate_lookups` (cada consulta é cobrada), limite diário por membro (promotora 20 / demais 100), promotora recebe só marca/modelo/ano/cor/combustível (chassi/Renavam só admin/comercial/closer), avisa se a placa já foi captada (`seller_vehicle_by_plate`). UI: campo Placa + "Buscar" em `CaptureNewLead` (opcional; autopreenche), botão "Consultar placa" nos Dados do contrato do `IntermediationCard` (preenche chassi/Renavam também). `create_capture_lead` grava `plate/plate_last4/color/fuel`. Token do tenant Totex já gravado. Hook `useVehicleLookup`. Placa é opcional (confirmado). **BLOQUEIO (2026-09-12):** o deploy da edge fn `vehicle-lookup` falhou com `402 Max number of functions reached for project` — o projeto bateu no teto de Edge Functions (~100). Migration + UI já estão em prod; só a function não subiu. Pra não deixar o CI vermelho a cada push, a pasta `supabase/functions/vehicle-lookup/` e o bloco dela no `config.toml` foram REMOVIDOS temporariamente (código salvo no histórico do commit 0de1937 e em `scratchpad/held/vehicle-lookup.index.ts`). **Assim que o Marco subir o limite no Supabase:** restaurar a pasta + o bloco no config.toml (`git checkout 0de1937 -- supabase/functions/vehicle-lookup`), recolocar `[functions.vehicle-lookup] verify_jwt = true`, commitar → o deploy sobe a function. Enquanto isso o botão "Buscar placa" na UI retorna 404 (erro em toast; cadastro manual segue funcionando).
