...
10. **Melhorias (kanban interno, 2026-07-12):** port da feature do vídeo do Frank. Botão flutuante 💡 na borda direita (todas as telas, badge = nº de novos) → painel "Reportar melhoria" (captura rota/URL/tela/browser + prints via getDisplayMedia ou upload, categoria, severidade) → kanban `/gestao/melhorias` (Novo → Eu peguei → Resolvido → Não vai rolar; drag nativo; filtros; detalhe com prints/contexto/notas de resolução). Tabela `melhoria_reports` (RLS tenant) + bucket privado `melhorias-prints` (signed URLs) — migration `20260712120000` APLICADA em produção. Só interno por enquanto (integração com grupo WhatsApp fica pra depois, decisão do Marco).
