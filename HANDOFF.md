...
4. **Marketplace — PONTE 1 IMPLEMENTADA (2026-09):** o marketplace ganhou o módulo **Negócios (Deal)** (`POST /api/deals/intake`, registro ponta a ponta da franquia) e envia cada negócio captado ao CRM com `evento: "NEGOCIO_CAPTADO"` (mesmo webhook/segredo `MARKETPLACE_WEBHOOK_SECRET`). O `marketplace-lead-webhook` trata esse evento assim: **sempre no tenant do Stand** (`config.stand_agent_config.stand_tenant_id` → fallback `tenants.is_super_admin`), idempotente por `metadata.totex_deal_id`, **dedup por telefone** (90 dias; mescla no lead existente e acumula `metadata.totex_deals[]`), loja dona em `metadata.owner_tenant_id` (via `marketplace_store_mappings`), veículo em `metadata.vehicle` (mesmo shape do `VehicleOfInterestCard`), eixos da franquia em `metadata.totex_origin/totex_channel/totex_kind/totex_intent`. **Abertura automática no WhatsApp oficial** (sessão do agente + template `stand_template_name`) só com `"marketplace_auto_open": true` em `config.stand_agent_config` — default desligado. Os leads aparecem em `/comercial/marketplace` (`source = "marketplace"`, `marketplace_origin = "NEGOCIO_CAPTADO"`). O fluxo antigo `INTERESSE_VEICULO` (clique no carro → tenant da loja) **continua igual**; migrar os leads do site para o intake de negócios é o próximo passo (Ponte 1b). Ponte 2 (CRM → marketplace: qualificação, handoff e ganho/perdido viram eventos do Deal) e Ponte 3 (tool `agendar_encontro` do Ronaldo) estão desenhadas em `docs/franquia/arquitetura-agentes-intermediacao.md` do repo `totexmotors-marketplace`.
