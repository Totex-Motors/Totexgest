...
5. **Mensagem vazia do assistant** no runner (aparece em `agents_messages` com content "") — regra de prompt não resolveu; investigar no `agent-runner` (provável text block vazio junto de tool call). Checar se chega vazia no WhatsApp do cliente.
