...
ON CONFLICT (tenant_id, slug) DO UPDATE SET display_name=EXCLUDED.display_name, description=EXCLUDED.description, system_prompt=EXCLUDED.system_prompt, settings=EXCLUDED.settings, template_variables=EXCLUDED.template_variables, is_template=true;
...
INSERT INTO public.agents_tools (agent_id, name, description, parameters_schema, action_type, action_config, usage_mode, is_active, provider) SELECT id, 'execute_sql', 'Roda SELECT (read-only) com auto-LIMIT 1000 e statement_timeout 30s. Bloqueado: INSERT/UPDATE/DELETE/DROP e tabelas sensíveis.', '{"type": "object", "required": ["sql"], "properties": {"sql": {"type": "string", "description": "Query SELECT/WITH apenas. Não use ; no meio."}}}'::jsonb, 'sql', '{"function": "agent_execute_readonly", "params_map": {"p_sql": "{{sql}}", "p_user_id": "{{user_id}}", "p_agent_id": "{{agent_id}}", "p_tenant_id": "{{tenant_id}}"}}'::jsonb, 'always', true, NULL FROM agents_registry WHERE slug = 'template_gestor_time';
...
INSERT INTO public.agents_tools (agent_id, name, description, parameters_schema, action_type, action_config, usage_mode, is_active, provider) SELECT id, 'team_roster', 'Retorna os membros ativos do time com nome e telefone. Use pra descobrir o telefone de quem você precisa MARCAR (@menção) no grupo, ou cruzar o responsável de uma pendência com o telefone dele. Não expõe email nem dados sensíveis.', '{"type": "object", "properties": {}}'::jsonb, 'sql', '{"function": "agent_team_roster", "params_map": {"p_agent_id": "{{agent_id}}", "p_tenant_id": "{{tenant_id}}"}}'::jsonb, 'always', true, NULL FROM agents_registry WHERE slug = 'template_gestor_time';
