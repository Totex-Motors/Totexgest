...
BEGIN
  ...
  IF NOT p_allow_regression THEN
    ...
    IF v_target_position < v_current_position THEN
      INSERT INTO ai_critical_decisions (lead_id, agent_id, deal_id, conversation_id, decision_type, decision, reason, severity, snapshot_data, requires_review, tenant_id)
      VALUES (v_lead_id, p_agent_id, p_deal_id, p_session_id, 'change_stage', 'blocked_regression',
        ...
        agent_resolve_tenant(NULL, p_agent_id));
  ...
  INSERT INTO ai_critical_decisions (lead_id, agent_id, deal_id, conversation_id, decision_type, decision, reason, severity, snapshot_data, tenant_id)
...
BEGIN
  ...
  INSERT INTO ai_critical_decisions (lead_id, agent_id, conversation_id, decision_type, decision, reason, severity, snapshot_data, tenant_id)
  VALUES (v_lead_id, p_agent_id, p_session_id, 'meeting_confirmed', 'success', 'Lead confirmou', 'low',
    jsonb_build_object('activity_id',p_activity_id), agent_resolve_tenant(NULL, p_agent_id));
...
BEGIN
  ...
  v_tenant := agent_resolve_tenant(p_user_id, p_agent_id);
...
BEGIN
  v_tenant := agent_resolve_tenant(NULL, p_agent_id);
  INSERT INTO agent_jobs (agent_id, session_id, channel, tool_name, external_id, poll_config, provider, resume_context, tenant_id)
  VALUES (p_agent_id, p_session_id, p_channel, p_tool_name, p_external_id, p_poll_config, p_provider, p_resume_context, v_tenant)
...
BEGIN
  ...
  INSERT INTO leads (name, phone, email, source, sales_stage, pipeline_stage_id, status, sales_rep_id, tenant_id)
  VALUES (trim(p_name), p_phone, p_email, COALESCE(p_source,'agente'), 'new', v_stage, 'active', p_user_id, v_tenant)
...
CREATE OR REPLACE FUNCTION public.agent_execute_readonly(p_sql text, p_user_id uuid DEFAULT NULL::uuid, p_agent_id uuid DEFAULT NULL::uuid, p_tenant_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
DECLARE
  ...
  v_keyword text; v_blacklist text[]; v_blocked text; v_result jsonb; v_tenant uuid;
  v_t0 timestamptz := clock_timestamp();
BEGIN
  -- Multi-tenant: a query roda SOB A RLS do tenant resolvido. Sem isso, um SELECT
  -- livre via service role enxergaria leads/deals de TODAS as lojas (vazamento).
  v_tenant := agent_resolve_tenant(p_user_id, p_agent_id, p_tenant_id);
  ...
  BEGIN
    ...
    -- Troca pro papel 'authenticated' (SEM bypass de RLS) + injeta o claim do tenant,
    -- de forma que get_tenant_id() volte v_tenant e as policies de leads/deals/etc
    -- filtrem automaticamente. Tudo SET LOCAL → reseta no fim da transação (1 RPC = 1 tx).
    PERFORM set_config(
      'request.jwt.claims',
      json_build_object('role','authenticated','app_metadata', json_build_object('tenant_id', v_tenant))::text,
      true
    );
    SET LOCAL ROLE authenticated;
    EXECUTE format('SELECT jsonb_agg(row_to_json(t)) FROM (%s) t', v_clean) INTO v_result;
    RESET ROLE;
    RETURN jsonb_build_object('data',COALESCE(v_result,'[]'::jsonb),'count',jsonb_array_length(COALESCE(v_result,'[]'::jsonb)),'duration_ms',extract(epoch FROM (clock_timestamp()-v_t0))*1000,'sql',v_clean,'tenant_id',v_tenant);
...
CREATE OR REPLACE FUNCTION public.agent_get_credential_data(p_provider_type text, p_owner_user_id uuid DEFAULT NULL::uuid, p_tenant_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  -- Multi-tenant: quando p_tenant_id é informado, só usa credenciais DAQUELA loja
  -- (evita um agente da loja A gastar a API key / billing da loja B).
...
BEGIN
  ...
  INSERT INTO ai_critical_decisions (lead_id, agent_id, deal_id, conversation_id, decision_type, decision, reason,
    severity, requires_review, snapshot_data, tenant_id)
  VALUES (v_lead_id, p_agent_id, p_deal_id, p_session_id, 'mark_lost', 'success', p_reason, 'high', true,
    jsonb_build_object('previous_stage_id', v_old_stage), agent_resolve_tenant(NULL, p_agent_id));
...
BEGIN
  ...
  INSERT INTO ai_critical_decisions (lead_id, agent_id, deal_id, conversation_id, decision_type, decision, reason,
    severity, impact_value_brl, requires_review, snapshot_data, criteria_used, llm_model, llm_input, llm_output, tenant_id)
  VALUES (p_lead_id, p_agent_id, v_deal_id, p_session_id, 'qualification', v_classification, p_reason,
    ...
    p_criteria_used, p_llm_model, p_llm_input, p_llm_output, agent_resolve_tenant(NULL, p_agent_id))
...
BEGIN
  ...
  IF r.repeat_every_minutes IS NOT NULL THEN
    ...
    IF r.repeat_until IS NULL OR v_next <= r.repeat_until THEN
      INSERT INTO agent_reminders (agent_id, session_id, channel, fire_at, message, resume_context, repeat_every_minutes, repeat_until, tenant_id)
      VALUES (r.agent_id, r.session_id, r.channel, v_next, r.message, r.resume_context, r.repeat_every_minutes, r.repeat_until, r.tenant_id);
...
BEGIN
  ...
  VALUES (v_lead_id, p_agent_id, p_session_id, 'meeting_rescheduled', 'success', p_reason, 'medium',
    jsonb_build_object('activity_id',p_activity_id,'old_start',v_old_start,'new_start',p_new_start_at), agent_resolve_tenant(NULL, p_agent_id));
  RETURN jsonb_build_object('ok',true,'activity_id',p_activity_id,'old_start',v_old_start,'new_start',p_new_start_at);
END $function$
;

CREATE OR REPLACE FUNCTION public.agent_resolve_tenant(p_user_id uuid DEFAULT NULL::uuid, p_agent_id uuid DEFAULT NULL::uuid, p_tenant_id uuid DEFAULT NULL::uuid)
 RETURNS uuid
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  -- Multi-tenant: resolve o tenant da loja de forma DETERMINÍSTICA.
  -- Prioridade: tenant explícito > tenant do AGENTE > tenant do usuário > JWT (não-default).
  -- IMPORTANTe: o pack original caía pro "tenant mais comum" quando não achava — em
  -- banco multi-loja isso VAZAVA dados entre lojas. Aqui, se não resolver, ERRA alto.
  -- Obs: as RPCs do agente são chamadas pelo runner via SERVICE ROLE, então
  -- get_tenant_id() volta o default (...001) — por isso preferimos o tenant do agente.
  v_tenant := COALESCE(
    p_tenant_id,
    (SELECT tenant_id FROM agents_registry WHERE id = p_agent_id),
    (SELECT tenant_id FROM team_members WHERE id = p_user_id AND tenant_id IS NOT NULL LIMIT 1),
    NULLIF(get_tenant_id(), '00000000-0000-0000-0000-000000000001'::uuid)
  );
  IF v_tenant IS NULL THEN
    RAISE EXCEPTION 'agent_resolve_tenant: não foi possível resolver o tenant (user=%, agent=%, tenant=%). '
      'Passe p_agent_id (recomendado) ou p_tenant_id.', p_user_id, p_agent_id, p_tenant_id;
  END IF;
  RETURN v_tenant;
END $function$
;

CREATE OR REPLACE FUNCTION public.agent_route_lookup(p_channel text, p_instance_id text, p_ctx jsonb DEFAULT '{}'::jsonb, p_tenant_id uuid DEFAULT NULL::uuid)
...
BEGIN
  ...
  FOR v_row IN
    ...
    WHERE d.channel = p_channel AND d.is_active AND r.is_active
      -- Multi-tenant: só casa deployments DA loja (evita rotear pra agente de outra loja
      -- na mesma instância/canal). p_tenant_id deve vir resolvido pela instância no webhook.
...
CREATE OR REPLACE FUNCTION public.agent_save_note(p_agent_id uuid, p_user_id uuid, p_title text, p_content text, p_mode text DEFAULT 'overwrite'::text, p_tags text[] DEFAULT NULL::text[])
 ...
 SET search_path TO 'public'
AS $function$
DECLARE v_slug text; v_existing record; v_final_content text; v_id uuid; v_tenant uuid;
...
BEGIN
  ...
  IF v_existing IS NULL THEN
    INSERT INTO agent_notes (agent_id, owner_user_id, title, slug, content, tags, metadata, tenant_id)
    VALUES (p_agent_id, p_user_id, p_title, v_slug, p_content, COALESCE(p_tags,'{}'), jsonb_build_object('last_author', v_author), v_tenant)
    RETURNING id INTO v_id;
...
CREATE OR REPLACE FUNCTION public.agent_schedule_followup(p_lead_id uuid, p_agent_id uuid, p_scheduled_for timestamp with time zone, p_message_brief text, p_reason text DEFAULT NULL::text, p_session_id uuid DEFAULT NULL::uuid)
 ...
 SET search_path TO 'public'
AS $function$
DECLARE v_followup_id uuid; v_tenant uuid;
BEGIN
  v_tenant := agent_resolve_tenant(NULL, p_agent_id);
  INSERT INTO ai_agent_scheduled_followups (lead_id, agent_id, scheduled_at, status, context_note, tenant_id)
  VALUES (p_lead_id, p_agent_id, p_scheduled_for, 'pending',
    COALESCE(p_message_brief,'') || CASE WHEN p_reason IS NOT NULL THEN E'\n[reason] '||p_reason ELSE '' END, v_tenant)
...
CREATE OR REPLACE FUNCTION public.agent_schedule_meeting(p_lead_id uuid, p_agent_id uuid, p_closer_id uuid, p_start_at timestamp with time zone, p_duration_minutes integer DEFAULT 30, p_title text DEFAULT NULL::text, p_notes text DEFAULT NULL::text, p_session_id uuid DEFAULT NULL::uuid)
 ...
 SET search_path TO 'public'
AS $function$
DECLARE v_activity_id uuid; v_existing_id uuid; v_lead_name text; v_tenant uuid;
BEGIN
  v_tenant := agent_resolve_tenant(NULL, p_agent_id);
  ...
  INSERT INTO company_activities (lead_id, responsavel_id, task_type, name, description,
    scheduled_at, duration_minutes, completed, team, metadata, tenant_id)
  VALUES (p_lead_id, p_closer_id, 'meeting',
    ...
    jsonb_build_object('created_by','agent_v2','agent_id',p_agent_id), v_tenant)
  RETURNING id INTO v_activity_id;
  INSERT INTO ai_critical_decisions (lead_id, agent_id, conversation_id, decision_type, decision, reason, severity, snapshot_data, tenant_id)
  VALUES (p_lead_id, p_agent_id, p_session_id, 'meeting_scheduled', 'success', 'Reuniao criada via agente', 'medium',
    jsonb_build_object('activity_id',v_activity_id,'closer_id',p_closer_id,'start_at',p_start_at,'duration_minutes',p_duration_minutes), v_tenant);
...
BEGIN
  ...
  VALUES (p_agent_id, p_session_id, COALESCE(p_channel,'chat_web'), p_fire_at, p_message,
    ...
    p_repeat_every_minutes, p_repeat_until, v_tenant)
  RETURNING id INTO v_id;
...
CREATE OR REPLACE FUNCTION public.agent_skill_my_hot_leads(p_user_id uuid, p_limit integer DEFAULT 20)
 ...
 SET search_path TO 'public'
AS $function$
  SELECT l.id, l.name, l.phone, l.sales_score::text, l.sales_score_reason, l.created_at
  FROM leads l
...
CREATE OR REPLACE FUNCTION public.agent_skill_notify_human(p_agent_id uuid, p_user_id uuid, p_session_id uuid, p_reason text)
 ...
 SET search_path TO 'public'
AS $function$
  INSERT INTO agents_action_log (agent_id, session_id, user_id, tool_name, input, status, tenant_id)
  VALUES (p_agent_id, p_session_id, p_user_id, 'notify_human', jsonb_build_object('reason', p_reason), 'pending_approval',
    agent_resolve_tenant(p_user_id, p_agent_id))
  RETURNING id;
$function$
;
...
CREATE OR REPLACE FUNCTION public.agent_team_roster(p_tenant_id uuid DEFAULT NULL::uuid, p_agent_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(member_id uuid, name text, phone text, role text, is_active boolean)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  -- Multi-tenant: só lista o time DA loja do agente (antes vazava todos os
  -- team_members de todas as lojas). Tenant resolvido pelo agente quando não vier explícito.
  SELECT id, name, phone, role, is_active
  FROM team_members
  WHERE is_active = true AND phone IS NOT NULL AND phone <> ''
    AND tenant_id = public.agent_resolve_tenant(NULL, p_agent_id, p_tenant_id)
  ORDER BY name;
$function$
;
...
BEGIN
  ...
  INSERT INTO ai_critical_decisions (lead_id, agent_id, conversation_id, decision_type, decision, reason, severity, snapshot_data, tenant_id)
  VALUES (p_lead_id, p_agent_id, p_session_id, 'update_lead', 'success', p_reason, 'low', v_applied, agent_resolve_tenant(NULL, p_agent_id));
