-- FIX de escopo: no painel "Meus leads" a promotora estava vendo os compradores
-- de TODO MUNDO (leads da rede, inclusive vendidos de outras promotoras).
--
-- Causa: list_my_buyer_leads usava is_superadmin(), que retorna true para QUALQUER
-- membro cujo tenant tem is_super_admin=true. Como as promotoras vivem no tenant
-- master (HQ, que é super_admin), todas eram tratadas como superadmin → v_target
-- virava NULL → a função devolvia a rede inteira.
--
-- Correção: o gate "vê todos" passa a ser por PAPEL REAL da pessoa (role='admin'
-- ou a flag pessoal is_superadmin no próprio membro), NUNCA pela flag do tenant.
-- Promotora (role='promotora') volta a ver só os leads que ELA captou.
...
BEGIN
  ...
  -- gate por PAPEL REAL, não pelo tenant (senão toda promotora do HQ vira admin).
  SELECT EXISTS (
