...
-- Tenant-scoped (mesmo padrão multi-tenant do 0001): cada loja só vê as próprias linhas
DO $$ BEGIN
  CREATE POLICY ap_tenant_read ON public.agents_versions FOR SELECT TO authenticated USING (tenant_id = public.get_tenant_id());
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  CREATE POLICY ap_tenant_write ON public.agents_versions FOR ALL TO authenticated USING (tenant_id = public.get_tenant_id()) WITH CHECK (tenant_id = public.get_tenant_id());
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  CREATE POLICY ap_tenant_read ON public.agents_logs FOR SELECT TO authenticated USING (tenant_id = public.get_tenant_id());
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  CREATE POLICY ap_tenant_read ON public.agents_core_memory FOR SELECT TO authenticated USING (tenant_id = public.get_tenant_id());
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  CREATE POLICY ap_tenant_write ON public.agents_core_memory FOR ALL TO authenticated USING (tenant_id = public.get_tenant_id()) WITH CHECK (tenant_id = public.get_tenant_id());
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
