...
-- ─── 2.15 Critical decisions (audit das decisões do agente sobre lead/deal) ───
-- Tabela esperada pelas RPCs de venda do 0003 (qualify/change_stage/mark_lost/etc).
-- Não existe no CRM base → criada aqui como parte da plataforma de agentes.
CREATE TABLE IF NOT EXISTS public.ai_critical_decisions (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL DEFAULT public.get_tenant_id(),
  lead_id uuid,
  agent_id uuid,
...
CREATE INDEX IF NOT EXISTS idx_ai_critical_decisions_tenant ON public.ai_critical_decisions(tenant_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_ai_critical_decisions_lead ON public.ai_critical_decisions(lead_id);
...
CREATE INDEX IF NOT EXISTS idx_agents_registry_tenant_slug ON public.agents_registry(tenant_id, slug);
...
CREATE INDEX IF NOT EXISTS idx_agents_deployments_channel_active
  ON public.agents_deployments(tenant_id, channel) WHERE is_active;
CREATE INDEX IF NOT EXISTS idx_agents_sessions_agent_user ON public.agents_sessions(agent_id, user_id);
CREATE INDEX IF NOT EXISTS idx_agents_sessions_tenant ON public.agents_sessions(tenant_id);
...
BEGIN
  FOREACH t IN ARRAY ARRAY[
    ...
    'ai_critical_decisions'
  ] LOOP
    EXECUTE format('ALTER TABLE public.%I ENABLE ROW LEVEL SECURITY', t);
  END LOOP;
END $$;

-- Policies MULTI-TENANT: cada loja só enxerga/edita as próprias linhas
-- (filtro por get_tenant_id(), idêntico ao padrão das tabelas-base do CRM).
-- Catálogos globais (skill_catalog, integration_providers) ficam legíveis a todos.
-- Edge functions agem com service_role (bypassa RLS) e resolvem o tenant explicitamente.
DO $$
DECLARE
  t text;
  has_pol boolean;
  v_read_using text;
  -- tabelas de DADOS do tenant (têm coluna tenant_id)
  tenant_tables text[] := ARRAY[
    'agents_registry','agents_tools','agents_deployments','agents_sessions',
    'agents_messages','agents_provider_credentials','agents_action_log',
    'agent_jobs','agent_reminders','agent_notes','agent_note_versions','agent_routing_log',
    'ai_critical_decisions'
  ];
  -- catálogos GLOBAIS (sem tenant_id) — leitura compartilhada
  global_tables text[] := ARRAY['agents_skill_catalog','agents_integration_providers'];
BEGIN
  -- ── Tabelas com tenant_id ──
  FOREACH t IN ARRAY tenant_tables LOOP
    -- agents_registry libera também os templates globais (is_template) pra todos
    IF t = 'agents_registry' THEN
      v_read_using := 'tenant_id = public.get_tenant_id() OR is_template = true';
    ELSE
      v_read_using := 'tenant_id = public.get_tenant_id()';
    END IF;

    SELECT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename=t AND policyname='ap_tenant_read') INTO has_pol;
    IF NOT has_pol THEN
      EXECUTE format('CREATE POLICY ap_tenant_read ON public.%I FOR SELECT TO authenticated USING (%s)', t, v_read_using);
    END IF;

    -- write (frontend edita): só nas tabelas que a UI altera; sempre dentro do tenant
    IF t IN ('agents_registry','agents_tools','agents_deployments','agents_provider_credentials',
             'agent_notes','agent_reminders') THEN
      SELECT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename=t AND policyname='ap_tenant_write') INTO has_pol;
      IF NOT has_pol THEN
        EXECUTE format(
          'CREATE POLICY ap_tenant_write ON public.%I FOR ALL TO authenticated USING (tenant_id = public.get_tenant_id()) WITH CHECK (tenant_id = public.get_tenant_id())',
          t);
      END IF;
    END IF;
  END LOOP;

  -- ── Catálogos globais (leitura a todos; escrita só service_role/admin) ──
  FOREACH t IN ARRAY global_tables LOOP
    SELECT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename=t AND policyname='ap_global_read') INTO has_pol;
    IF NOT has_pol THEN
      EXECUTE format('CREATE POLICY ap_global_read ON public.%I FOR SELECT TO authenticated USING (true)', t);
    END IF;
  END LOOP;
END $$;
