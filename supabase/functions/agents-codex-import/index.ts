...
serve(async (req) => {
  ...
  // Multi-tenant: a credencial pertence à loja do usuário que importou (JWT).
  const tenantId = getTenantIdFromRequest(req) ?? payload.tenant_id ?? null;
  if (!tenantId) {
    return jsonResp({ ok: false, error: "tenant_id não resolvido (faça login na loja antes de importar a credencial)" }, 400);
  }

  // Idempotente: procura por credencial existente desse account_id NA MESMA loja
