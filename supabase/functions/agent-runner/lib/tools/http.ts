...
export async function execHttpTool(params: {
  ...
  user_id: string | null;
  /** Loja dona do agente — escopa a credencial do provider (billing/segurança). */
  ...
  if (params.provider) {
    const credData = await resolveCredential(params.provider, params.user_id, params.tenant_id ?? null);
    if (credData) {
...
async function resolveCredential(
  ...
  // Cache indexado por tenant — senão a credencial de uma loja vazaria pra outra
  // no mesmo isolate (risco de billing/segurança).
  const cacheKey = `${tenantId || "_"}::${providerType}::${ownerUserId || "_"}`;
