...
export async function executeTool(params: ExecuteToolParams): Promise<ExecuteToolResult> {
  ...
  // Loja dona da conversa — injetada pelo runner no runtime_context. Usada pra
  // gravar o audit log no tenant certo e escopar credenciais de skills HTTP.
  const tenant_id = (ctx.tenant_id as string | null | undefined) ?? null;
  ...
  try {
    ...
    if (asyncCfg && tool.action_type === "http") {
      ...
      db.from("agents_action_log").insert({
        tenant_id, agent_id, session_id, user_id,
