...
export interface CallProviderParams {
  agent: AgentRegistry;
  /** Loja dona do agente — escopa a busca da credencial (billing/segurança). */
...
export async function callProvider(
  ...
  if (!credential && params.agent.credential_id) {
    // service role bypassa RLS → filtra tenant_id explicitamente pra não usar a
    // credencial (API key / billing) de outra loja, mesmo com credential_id adulterado.
    let q = db
