...
Deno.serve(async (req: Request) => {
  ...
  // ────────── 0. Resolve o tenant (loja) ──────────
  // chat_web: vem no JWT do usuário (header Authorization). Webhooks/cron (WhatsApp,
  // Telegram, poller): passam tenant_id explícito no body. SEM tenant não roteamos —
  // senão um agente de slug repetido entre lojas seria ambíguo / vazaria dados.
  const tenantId = payload.tenant_id ?? getTenantIdFromRequest(req);
  if (!tenantId) {
    return jsonError(
      "tenant_id não resolvido. chat_web precisa do JWT do usuário; webhooks/cron devem enviar tenant_id no body.",
      400,
    );
  }

  // ────────── 1. Carrega agente (escopado ao tenant) ──────────
  // service role bypassa RLS → o filtro de tenant_id é OBRIGATÓRIO aqui.
  ...
  if (agentErr || !agentRow) {
    return jsonError(`Agente '${payload.agent_slug}' não encontrado para esta loja`, 404);
  }
          ...
          const toolResults = await Promise.allSettled(
            result.tool_calls.map(async (call: ToolCall) => {
              ...
              const execResult = await executeTool({
                ...
                user_id: payload.user_id || null,
                runtime_context: {
                  // Channel + tenant (loja) + tudo do payload.context (lead_id, deal_id, instance_id, message_id, etc).
                  // tenant_id é injetado aqui pra que as tools SQL resolvam {{tenant_id}} e operem na loja certa.
                  channel: payload.channel || "chat_web",
                  tenant_id: tenantId,
