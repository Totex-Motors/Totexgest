...
serve(async (req) => {
  ...
  // Multi-tenant: resolve a chave Gemini DA loja dona da nota (fallback global/env).
  let tenantId: string | null = null;
  try {
    const { data: noteRow } = await supabase.from(table).select("tenant_id").eq("id", row_id).maybeSingle();
    tenantId = (noteRow as { tenant_id?: string } | null)?.tenant_id ?? null;
