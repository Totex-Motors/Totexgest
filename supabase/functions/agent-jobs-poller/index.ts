...
async function checkJob(job: any): Promise<{ status: string }> {
  ...
  if (poll.headers) {
    const cred = job.provider ? await getCredential(job.provider, job.tenant_id ?? null) : null;
    ...
    const cred = await getCredential(job.provider, job.tenant_id ?? null);
...
async function getCredential(providerType: string, tenantId: string | null = null): Promise<Record<string, unknown> | null> {
  const { data } = await supabase.rpc("agent_get_credential_data", { p_provider_type: providerType, p_owner_user_id: null, p_tenant_id: tenantId });
