...
serve(async (req) => {
  ...
  try {
    ...
    const { data: dep } = await db
      ...
      .select("id, agent_id, tenant_id, config, is_active, agents_registry(slug)")
      .eq("id", deploymentId)
      .maybeSingle();
    if (!dep || !dep.is_active) return ok();
    const tenantId = (dep as any).tenant_id as string | null;
    if (!tenantId) { console.error("[telegram] deployment sem tenant_id"); return ok(); }
