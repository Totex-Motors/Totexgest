  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      for (let i = 0; i < pixels.length; i += 4) {
        ...
        if (alpha > 0) {
          // The logo is black on transparent — replace dark pixels with teal
...
export async function generateCertificate(data: CertificateData): Promise<void> {
  ...
  // White canvas + teal accents + strong typography
  // =============================================
  ...
  // === LEFT ACCENT BAR ===
  const barWidth = 16;
  doc.setFillColor(0, 150, 137); // brand teal
  doc.rect(0, 0, barWidth, H, 'F');

  // Darker edge
  doc.setFillColor(0, 110, 100); // brand teal dark
