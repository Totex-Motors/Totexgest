...
const getColor = (type: string) => {
  switch (type) {
    ...
    case 'access': return 'bg-teal-500';
    ...
    case 'billing': return 'bg-teal-500';
                    ...
                    <div className="ml-4">
                      ...
                      <div className={`p-4 bg-muted/50 rounded-lg border transition-colors ${isClickable ? 'group-hover:bg-blue-50 group-hover:border-blue-200' : 'group-hover:bg-muted/70'}`}>
                        ...
                        {event.metadata?.source === 'book_meeting_page' && (
                          <div className="mt-3 p-3 bg-teal-50 dark:bg-teal-950/30 rounded-lg border border-teal-200 dark:border-teal-800 space-y-1">
                            <p className="text-xs font-medium text-teal-700 dark:text-teal-400 mb-1">📋 Dados do agendamento:</p>
                            {event.metadata.company && (
                              <p className="text-xs"><span className="font-semibold text-teal-600">Empresa:</span> {event.metadata.company}</p>
                            )}
                            {event.metadata.revenue && (
                              <p className="text-xs"><span className="font-semibold text-teal-600">Faturamento:</span> R$ {(event.metadata.revenue / 1000).toFixed(0)}k/mês</p>
                            )}
                            {event.metadata.evento && (
                              <p className="text-xs"><span className="font-semibold text-teal-600">Evento:</span> {event.metadata.evento}</p>
                            )}
                            {event.metadata.utm_source && (
                              <p className="text-xs"><span className="font-semibold text-teal-600">Origem:</span> {event.metadata.utm_source} / {event.metadata.utm_campaign || '-'}</p>
                            )}
                            {event.metadata.meeting_link && (
                              <a href={event.metadata.meeting_link} target="_blank" rel="noopener noreferrer" className="text-xs text-teal-600 hover:underline">🔗 Link da reunião</a>
                            )}
                          </div>
                        )}
                        {/* Prints/Anexos */}
